...
class nanoSQUID_DR(nanoSQUIDSystem):
    ...
    def configureEquipment(self):
        ...
        self.equip.add_server("nSOT DAC", "dac_adc", "DA10_16_08 (COM4)", config=dict)

        dict = {'x out':2, 'y out':3, 'z out':1}
        self.equip.add_server("Scan DAC", "dac_adc", "DA10_16_08 (COM4)", config=dict)

        self.equip.add_server("Sample DAC", "dac_adc", "DA10_16_08 (COM4)")

        dict = {'blink channel':0}
        self.equip.add_server("Blink Device", "dac_adc", "DA10_16_08 (COM4)", config=dict)
...
if __name__=="__main__":
    ...
    window = nanoSQUID_DR(reactor, computer='kraken')
    window.show()
    reactor.runReturn()
    sys.exit(app.exec_())
