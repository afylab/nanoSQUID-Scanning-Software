...
class gotoSetWindow(QtGui.QDialog, Ui_gotoSetpoint):
	def __init__(self, reactor, magPower, dac, blinkDev, sweepParams, dacouts, dacins, parent = None):
		...
		self.blinkBtn.clicked.connect(self.blinkFunc)
	...
	def ipsGoToFieldFunc(self, field, rate, c = None):
		...
		yield self.sleep(0.25)
		yield self.magDev.set_fieldsweep_rate(float(rate))
		yield self.magDev.set_control(2)
	...
	def blinkFunc(self, c = None):
		yield self.blinkDev.set_voltage(5)
		yield self.sleep(0.25)
		yield self.blinkDev.set_voltage(0)
		yield self.sleep(0.25)
...
class Window(QtGui.QMainWindow, Ui_MainWindow):
	...
	def checkSweep(self):
		...
		elif self.fieldSIStat == 'field steps':
	...
	def initSweep(self):
		...
		elif  self.sweepParamDict['sweep mode'] == 0:
			self.isData = True
			self.sweepFromZero(self.sweepParamDict['mag power'])


	@inlineCallbacks
	def abortSweep(self, magpower, bVal, vVal, c = None):
	...
	def sweepMinMax(self, magpower, c = None):
		...
		DAC_set_volt = self.dcOutputsDict['toe_volts'] - 1
		...
		if magpower == 'toellner':
			...
			for i in range(0, B_pnts):
				...
				if i == 0:
					...
					if self.abortFlag == False:
						...
						yield self.abortSweep('toe', 0, V_min)
						break
					...
					if self.abortFlag == False:
						...
						yield self.abortSweep('toe', B_space[i-1], V_min)
						break
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('toe', B_space[i], V_min)
					break
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('toe', B_space[i], V_max)
					break
			...
			for i in range(0, B_pnts):
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('ips', B_space[i], V_min)
					break
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('ips', B_space[i], V_max)
					break
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep(B_space[i],V_min)
					break
		...
		if magpower == 'toellner':
			for i in range (0, B_pnts):
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('toe', B_space[i], 0)
					break
			...
			for i in range (0, B_pnts):
				...
				if self.abortFlag == False:
					...
					yield self.abortSweep('ips', B_space[i], 0)
					break
...
class preliminarySweep(QtGui.QDialog, Ui_prelimSweep):
	...
	def plotSweepData(self, data):
		...
		xVals = [x[0] for x in self.data]
		yVals = [x[1] for x in self.data]
	...
	def sweep(self, c):
		...
		if self.btnAction == 'sweep':
			try:
				...
				#Sets DAC Channels
				DAC_out = self.dacOutDict['nsot bias'] - 1
				DAC_blink = self.dacOutDict['blink'] - 1
				
				DAC_in_ref = self.dacInDict['bias ref'] - 1
				DAC_in_sig = self.dacInDict['feedback'] - 1
				DAC_in_noise = self.dacInDict['noise'] - 1
